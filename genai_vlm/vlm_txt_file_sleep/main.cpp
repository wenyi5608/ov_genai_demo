...
int main(int argc, char* argv[]) try {
    ...
    if (!(test_mode == TestMode::memory) && !(test_mode == TestMode::performance)) {
        std::cout << "lora path " << adapter_path << std::endl;
    ...
    //mp.emplace(ov::hint::dynamic_quantization_group_size.name(), (size_t)0);
    //std::cout << "dynamic quantization is 0" << std::endl;

    mp.emplace(ov::hint::enable_cpu_pinning.name(), true);
    mp.emplace(ov::enable_profiling.name(), false); 
    //mp.emplace(ov::hint::kv_cache_precision.name(), ov::element::f16);
    mp.emplace(ov::intel_gpu::hint::queue_throttle.name(), ov::intel_gpu::hint::ThrottleLevel::MEDIUM);
    mp.emplace(ov::intel_gpu::hint::queue_priority.name(), ov::hint::Priority::MEDIUM);
    mp.emplace(ov::intel_gpu::hint::host_task_priority.name(),ov::hint::Priority::HIGH);
    ...
    for (int loop = 0; loop < 10; loop++) {
        std::cout << "loop index " << loop << std::endl;
        ...
        //sleep
        int loop_time = (loop + 1) * 120;
        std::cout << "sleep " << loop_time << " S" << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds((loop + 1) * 120));
